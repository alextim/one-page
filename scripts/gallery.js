...
Object.keys(srcWidthes)
  ...
  .forEach((key) => {
    ...
    jpgSrcSet += `        \${process.env.PUBLIC_URL}/${IMG_DIR}/${jpgToImport} ${w}w`;

    if (webpSrcSet) {
      webpSrcSet += ',\r\n';
    }
    webpSrcSet += `        \${process.env.PUBLIC_URL}/${IMG_DIR}/${webpToImport} ${w}w`;
  ...
  */
    console.log(`${srcDirectory}/${name}.jpg`);
    console.log(`${dstDirectory}/${jpgToImport}`);
...
const HeroImg = ({ alt }) => (
  <picture>
    ...
    <img
      sizes="${sizes}"
      srcSet={\`\r\n${jpgSrcSet}\r\n      \`}
      src={\`\${process.env.PUBLIC_URL}/${IMG_DIR}/${formatImgFileName(
  maxWidth,
  maxHeight,
  'jpg'
)}\`}
